...
class Server:
    def __init__(self, host, port):
        ...
        self.clients = {}  # Ngestore clients, addresses, dan usernames
            ...
            try:
                ...
                if message.startswith('LOGIN'):
                    ...
                    if username in self.clients.values():
                        self.sock.sendto('Username sudah dipakai. Gunakan yang lain.'.encode('ascii'), addr)
                    elif password != 'GAS1':
                        self.sock.sendto('Password salah. Silahkan coba lagi.'.encode('ascii'), addr)
                    else:
                        self.clients[addr] = username
                        self.sock.sendto(f'Welcome {username}!'.encode('ascii'), addr)
                        print(f'{username} sudah bergabung {addr}.')
                else:
